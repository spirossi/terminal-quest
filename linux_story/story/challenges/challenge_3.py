...
class Step1(StepTemplateCat):
    ...
    hints = [line.encode(
        'utf-8') for line in [_("{{rb:Type}} {{yb:ls shelves/}} {{rb:to look at your books.}}")]]
...
class Step2(StepTemplateCat):
    ...
    hints = [line.encode(
        'utf-8') for line in [_("{{rb:Type}} {{yb:cat shelves/comic-book}} {{rb:to read the comic.}}")]]
...
class Step3(StepTemplateCat):
    ...
    hints = [line.encode(
        'utf-8') for line in [_("{{rb:Type}} {{yb:cat shelves/note}} {{rb:to read the note.}}")]]
